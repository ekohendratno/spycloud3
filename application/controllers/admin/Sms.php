...
class Sms extends CI_Controller{
	...
	function index(){
        ...
        $this->template->load('template','admin/sms',$data);
	}
	...
	function cobaQuery($params = array()){
		...
		$data = array();
		$commands = $this->db->select('*')->from('sms');

        if(!empty($params['search']['sortBy'])){
            $this->db->order_by('tanggal',$params['search']['sortBy']);
        }else{
            $this->db->order_by('tanggal','desc');
        }

        if(!empty($params['search']['keywords'])){
            $this->db->like('message',$params['search']['keywords']);
		...
		foreach ($commands->result_array() as $row){
			$baris['sms_id'] = $row['sms_id'];
			$baris['address'] = $row['address'];
			$baris['message'] = $row['message'];
            $baris['tanggal'] = $row['tanggal'];
            $baris['reader'] = $row['reader'];
            $baris['id'] = $row['id'];
            $baris['type'] = $row['type'];
	...
	function ajaxPaginationData(){
        ...
        $config['base_url']    = base_url().'sms/ajaxPaginationData';
	...
	function hapusdatabyid(){
		...
		$this->m->hapusbyid(array('sms_id'=>$id),'sms');
