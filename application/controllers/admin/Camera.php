...
class Camera extends CI_Controller{
	...
	function index(){
        ...
        $this->template->load('template','admin/camera',$data);
	}
	...
	function cobaQuery($params = array()){
		...
		$data = array();
		$commands = $this->db->select('*')->from('camera');


        if(!empty($params['search']['phoneBy'])){
            $this->db->where('phone_id',$params['search']['phoneBy']);
        }

        if(!empty($params['search']['sortBy'])){
            $this->db->order_by('camera_date',$params['search']['sortBy']);
        }else{
            $this->db->order_by('camera_date','desc');
        }
		...
		foreach ($commands->result_array() as $row){
			$baris['camera_id'] = $row['camera_id'];
			$baris['image'] = "";

			if( file_exists("uploads/".$row['image'])){
                $baris['image'] = $row['image'];
            }

			$baris['for'] = $row['for'];
			$baris['camera_date'] = $row['camera_date'];
	...
	function ajaxPaginationData(){
        ...
        $config['base_url']    = base_url().'camera/ajaxPaginationData';
	...
	function hapusdatabyid(){
        ...
        $camera = $this->db->get_where('camera', array('camera_id'=> $id) )->result();
        foreach ($camera as $a){
		...
		$this->m->hapusbyid(array('camera_id'=>$id),'camera');
