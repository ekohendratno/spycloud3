...
class Contact extends CI_Controller{
	...
	function cobaQuery($params = array()){
		...
		$data = array();
		$commands = $this->db->select('*')->from('contacts');

        if(!empty($params['search']['sortBy'])){
            $this->db->order_by('contact_name',$params['search']['sortBy']);
        }else{
		...
		foreach ($commands->result_array() as $row){
			$baris['contacts_id'] = $row['contacts_id'];
			$baris['contact_name'] = $row['contact_name'];
			$baris['contact_phone'] = $row['contact_phone'];
	...
	function ajaxPaginationData(){
        ...
        $config['base_url']    = base_url().'contact/ajaxPaginationData';
	...
	function hapusdatabyid(){
		...
		$this->m->hapusbyid(array('contacts_id'=>$id),'contacts');
